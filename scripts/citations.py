...
Two shapes, chosen by `source_type`:
    ...
    caselaw   Shweika v. Dep't of Homeland Sec., 723 F.3d 710 (6th Cir. 2013)
...
# names ("Mazen Shweika v. ...") and spell out institutions ("United States Citizenship and
# Immigration Services"). Both are wrong in a citation, and neither can be fixed by rule —
# "keep the last word of the plaintiff" gives Diaz for Morfa Diaz, Dandrade for De Dandrade
# and Reis for Dos Reis, i.e. it invents a party name for 3 of 26 and does it silently.
#
# So the short names are hand-written and eyeballed against the caption, one line per opinion,
# keyed by `source_id` because that is the stable identifier. 26 lines is smaller than the
# abbreviation rules would be, and it cannot be subtly wrong. Anything unmapped falls back to
# the caption verbatim: long, but never a case name that does not exist.
CASE_NAMES = {
    "opinion_1035207_mazen_shweika_v_dept_of_homeland_security":
        "Shweika v. Dep't of Homeland Sec.",  # Mazen Shweika v. Dep't of Homeland Security
    ...
    "opinion_4416725_emad_haroun_v_us_dept_of_homeland_security":
        "Haroun v. U.S. Dep't of Homeland Sec.",  # Emad Haroun v. U.S. Dept of Homeland Security
    "opinion_4565506_moya_v_united_states_department_of_homeland_security":
        "Moya v. U.S. Dep't of Homeland Sec.",  # Moya v. United States Department of Homeland Security
    "opinion_4574859_northwest_immigrant_rights_project_v_united_states_citizenship_and_immigration_services":
        "Nw. Immigrant Rights Project v. USCIS",  # Northwest Immigrant Rights Project v. United States Citizenship and Immigration Services
    ...
    "opinion_625607_gonzalez_v_secretary_of_department_of_homeland_security":
        "Gonzalez v. Sec'y of Dep't of Homeland Sec.",  # Gonzalez v. Secretary of Department of HomeLand Security
    ...
    "opinion_7238503_iqbal_v_secretary_us_department_of_homeland_security":
        "Iqbal v. Sec'y, U.S. Dep't of Homeland Sec.",  # Iqbal v. Secretary U.S. Department of Homeland Security
    ...
    "opinion_7252751_de_dandrade_v_us_dept_of_homeland_sec":
        "De Dandrade v. U.S. Dep't of Homeland Sec.",  # De Dandrade v. U.S. Dep't of Homeland Sec.
    ...
    "opinion_7798407_elvis_leonel_morfa_diaz_v_acting_secretary_department_of_homeland_security":
        "Morfa Diaz v. Acting Sec'y, Dep't of Homeland Sec.",  # Elvis Leonel Morfa Diaz v. Acting Secretary, Department of Homeland Security
    ...
    "opinion_9781657_muhanad_alhasani_v_secretary_united_states_department_of_homeland_sec":
        "Al-Hasani v. Sec'y, U.S. Dep't of Homeland Sec.",  # Muhanad Al-Hasani v. Secretary United States Department of Homeland Sec
}
...
def case_name(metadata):
    """The hand-checked short name, falling back to the CourtListener caption."""
    return CASE_NAMES.get(metadata.get("source_id")) or metadata.get("title") or "Unknown case"


def case_citation(metadata):
    """`Shweika v. Dep't of Homeland Sec., 723 F.3d 710 (6th Cir. 2013)`.
